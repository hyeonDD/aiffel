...
자비스는 어떻게 개발됐는지, 무엇을 할 수 있는지, 그리고 AI에 대해 저커버그는 어떤 생각을 하고 있는지 살펴 보세요. 원문은 해당 링크([https://www.facebook.com/notes/mark-zuckerberg/building-jarvis/10154361492931634](https://www.facebook.com/notes/mark-zuckerberg/building-jarvis/10154361492931634))에서 확인하실 수 있습니다.
...
git 버전(2.28이후)부터는 기본 branch가 master->main으로 변경된걸 머리로는 알고 있었지만 주피터 환경의 git version(2.25.1)을 확인했을때 **또** 아무런 의심없이 git 명령어를 입력하다가 에러를 마주쳤었다..

```sh
# git push origin main
error: src refspec main does not match any
error: failed to push some refs to 'https://github.com/hyeonDD/aiffel.git'
```

이미 이전에 해결해봤던 에러였기에 쉽게 해결했지만 (git version을 업데이트 해도 되지만 그냥 branch만 변경)
...
# 글로벌하게 설정 (git init 명령어 사용시 디폴트 main)
$ git config --global init.defaultBranch main
```

이 방법으로 해결했고 이미 쪼끔 써봤던 툴이라 너무 자만, 성급했던거 같다. 앞으로는 더 꼼꼼함하게 봐야겠다.
